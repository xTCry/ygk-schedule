...
export const openCalendarDialog = (
  ...
  description.textContent = subgroup
    ? 'Выбрана конкретная подгруппа, поэтому ссылки ниже содержат только её пары и общие занятия. Скопируйте ссылку и добавьте её в Google Calendar: «Другие календари» → «По URL».'
    : 'Чтобы подписаться только на свою подгруппу, сначала выберите её на странице группы, затем снова откройте это окно. Актуальный календарь учитывает только подтверждённые замены.';
