...
Парсер и генераторы остаются CLI-утилитами; HTTP-сервера в проекте нет.
...
```text
https://xtcry.github.io/ygk-schedule/
```

На нём можно выбрать одну группу и увидеть ближайшие учебные дни. Сайт
показывает базовое расписание, применённые замены, отмены, дополнительные
необработанные строки замен и относящиеся к группе warnings/errors. Данные
обновляются автоматически, поэтому важные изменения следует сверять с
официальными источниками колледжа.

Статический JSON API не требует сервера:

```text
https://xtcry.github.io/ygk-schedule/api/index.json
https://xtcry.github.io/ygk-schedule/api/base/groups/СТ1-11.json
https://xtcry.github.io/ygk-schedule/api/actual/groups/СТ1-11.json
https://xtcry.github.io/ygk-schedule/api/replacements/groups/СТ1-11.json
```
...
Для каждой пары профиль звонков определяется по аудитории из расписания:
`А203` и `М102` используют профиль А/М, `Б504` и `В301` — Б/В. Корпуса Т и Ф
дополнительно уточняются по курсу или явно подтвержденному списку групп.
Правила находятся в [`config/ygk/calendar.yaml`](config/ygk/calendar.yaml),
полная таблица звонков и регламент — рядом в `bells.yaml` и
`regulations.yaml`. `ДОТ`, спортзал и неизвестные сокращения не получают
угаданное время: пара пропускается из ICS и фиксируется в результате
генерации. `CALENDAR_GROUP` нужен только для локальной проверки одной группы.
...
ICS уже публикуется через GitHub Pages. После изменения calendar-конфига
достаточно выполнить следующую генерацию ICS, чтобы свойства `SOURCE` и `URL`
в самих файлах указывали на Pages.

Локальный snapshot сайта из уже подготовленной папки `data/` собирается без
сетевых запросов:
...
Результат появится в `.tmp/pages/`: можно открыть его через любой локальный
статический сервер. В CI этот же build забирает актуальную ветку `data` и
публикует артефакт Pages.
